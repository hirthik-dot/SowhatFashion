...
export default function BillItemRow({
  ...
  return (
    <div className="grid grid-cols-12 gap-2 items-center bg-[var(--surface-2)] rounded p-2 text-sm">
      <div className="col-span-1">{index + 1}</div>
      <div className="col-span-3">{item.name} {item.size ? `(${item.size})` : ""}</div>
      <div className="col-span-1">₹{item.mrp}</div>
      ...
      <div className="col-span-1">₹{price.toFixed(2)}</div>
      <div className="col-span-2 flex items-center gap-1">
        <button className="h-9 w-9 rounded border border-[var(--border)]" onClick={() => onQty(item.quantity - 1)}>-</button>
        <span className="w-8 text-center">{item.quantity}</span>
        <button className="h-9 w-9 rounded border border-[var(--border)]" onClick={() => onQty(item.quantity + 1)}>+</button>
      </div>
      <div className="col-span-1">₹{total.toFixed(2)}</div>
      <button className="col-span-1 text-[var(--error)]" onClick={onRemove}>✕</button>
    </div>
  );
}
