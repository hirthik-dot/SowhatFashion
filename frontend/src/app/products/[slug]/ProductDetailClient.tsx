...
export default function ProductDetailClient({ product }: { product: any }) {
  ...
  const handleAddToCart = (): boolean => {
    ...
    const existingItem = currentItems.find(i => 
      i.productId === (product.parentProductId || product._id) && 
      i.size === selectedSize && 
      i.color === colorName
    );
    ...
    if (existingQuantity + quantity > stock) {
      if (existingQuantity > 0) {
        alert(`You can only add up to ${stock} items. You already have ${existingQuantity} in your cart.`);
      } else {
        alert(`You can only add up to ${stock} items.`);
      }
      return false;
    }
          ...
          {images.length > 0 ? (
            images.map((img: string, i: number) => (
              <button
                ...
                className={`relative w-20 h-24 bg-[var(--surface)] shrink-0 border-2 transition-all ${selectedImage === i ? 'border-[var(--gold)] opacity-100' : 'border-transparent opacity-60 hover:opacity-100'
                  }`}
              >
          ...
          {images.length > 1 && (
            <>
              ...
              <button
                ...
                className="absolute left-2 md:left-4 top-1/2 -translate-y-1/2 w-8 h-8 md:w-10 md:h-10 bg-white/80 hover:bg-white text-black rounded-full flex items-center justify-center opacity-100 shadow-md z-10 transition-colors"
                aria-label="Previous image"
              >
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5 md:w-6 md:h-6"><polyline points="15 18 9 12 15 6"></polyline></svg>
              </button>
              <button
                onClick={(e) => { e.preventDefault(); nextImage(); }}
                className="absolute right-2 md:right-4 top-1/2 -translate-y-1/2 w-8 h-8 md:w-10 md:h-10 bg-white/80 hover:bg-white text-black rounded-full flex items-center justify-center opacity-100 shadow-md z-10 transition-colors"
                aria-label="Next image"
              >
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5 md:w-6 md:h-6"><polyline points="9 18 15 12 9 6"></polyline></svg>
              </button>
            </>
          )}
                ...
                <div className="flex flex-wrap gap-3">
                  {sizes.map((size: string) => (
                    <button
                      ...
                      className={`nav-link w-12 h-12 flex items-center justify-center border transition-all rounded font-medium ${selectedSize === size
            ...
            <div className="mb-8">
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center border border-[var(--border)] rounded">
                  ...
                  <button onClick={() => setQuantity(Math.min(stock, quantity + 1))} className="px-4 py-3 hover:bg-gray-50 text-[var(--text-secondary)] transition-colors">+</button>
                </div>
