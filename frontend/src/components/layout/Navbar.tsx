...
export default function Navbar({ variant = 'default' }: NavbarProps) {
              ...
              <div className="relative flex items-center gap-0.5" ref={dropdownRef}>
                {isLoggedIn && user ? (
                  <>
                    <Link
                      href="/account"
                      className="w-8 h-8 rounded-full bg-[var(--gold)] text-black font-bold flex items-center justify-center text-[10px] overflow-hidden shrink-0 border border-black/10"
                      title="My Account"
                    ...
                    <button
                      ...
                      className="p-0.5 rounded hover:text-[var(--gold)] text-[var(--text-secondary)]"
                    >
                      <ChevronDown size={18} className={dropdownOpen ? 'rotate-180 transition-transform' : 'transition-transform'} />
                    </button>
                  </>
                ) : (
                  <button type="button" onClick={() => openAuthModal()} className="hover:text-[var(--gold)] transition-colors mt-1">
                    <UserIcon size={20} />
                  </button>
                )}
                {dropdownOpen && isLoggedIn && user && (
                  <div className="absolute right-0 top-full mt-1 w-52 bg-white rounded-lg shadow-xl py-2 border border-[var(--border)] z-50">
  ...
  return (
    <>
      <nav className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white shadow-md' : 'bg-white border-b border-[var(--border)]'
        }`}>
          ...
          <div className="flex items-center justify-between h-16 md:h-18">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="relative flex items-center gap-0.5" ref={dropdownRef}>
                ...
                {dropdownOpen && isLoggedIn && user && (
                  <div className="absolute right-0 top-full mt-1 w-52 bg-[var(--bg)] rounded-lg shadow-xl py-2 border border-[var(--border)] z-50">
                    <div className="px-4 py-2 border-b border-[var(--border)] text-sm font-semibold text-black">
                      Hi, {userFirstName(user)}
                    </div>
                    <Link href="/account" onClick={() => setDropdownOpen(false)} className="block px-4 py-2 text-sm text-[var(--text-secondary)] hover:bg-[#C9A84C]/15 hover:text-[var(--gold)]">
        ...
        <div className={`fixed inset-0 z-[100] transition-opacity duration-300 md:hidden ${menuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
          ...
          <div className={`absolute top-0 left-0 w-[80%] max-w-[300px] h-full bg-white transform transition-transform duration-300 ease-in-out flex flex-col ${menuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            ...
            <div className="p-6 border-t border-[var(--border)]">
              <a href={WHATSAPP_LINK} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-green-600 font-medium">
