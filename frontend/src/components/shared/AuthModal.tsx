...
export default function AuthModal() {
  ...
  const handleSendOtp = async (e?: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/auth/send-otp`, {
        ...
        body: JSON.stringify({ email: emailNorm, purpose: 'login' })
      });
  ...
  const handleVerifyOtp = async (otpValue: string) => {
    ...
    const code = String(otpValue).replace(/\D/g, '').slice(0, 6);
    ...
    setLoading(true);
    try {
      const payload = { email: emailNorm, otp: code, purpose: 'login' as const };
